...
@injectable()
export default class ObterPorNumeroInstalacaoFazendaUseCase extends UseCase<
  string,
  FazendaEntity
> {
  constructor(
  ...
  async execute(data: string): Promise<any> {
    const exists = await this.repository.findByFazendaNumeroInstacao(data);

    if (!exists) {
      throw new NotFoundException(FazendaEnum.FAZENDA_NAO_ENCONTRADA);
    }

    const result = await this.repository.findByFazendaNumeroInstacao(data);
