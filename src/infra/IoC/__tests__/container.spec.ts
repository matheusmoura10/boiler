...
describe("Dependency Injection Container Tests", () => {
  ...
  it("deve resolver as dependências corretamente", () => {
    const concessionariaModel = diContainer.get<ConcessionariaModel>(
      "ConcessionariaModel"
    );
    expect(concessionariaModel).toBeInstanceOf(ConcessionariaModel);

    const concessionariaRepository = diContainer.get(
      "ConcessionariaRepository"
    );
    expect(concessionariaRepository).toBeInstanceOf(ConcessionariaRepository);

    const saveConcessionariaUsecase = diContainer.get(
      "SaveConcessionariaUsecase"
    );
    expect(saveConcessionariaUsecase).toBeInstanceOf(SaveConcessionariaUsecase);

    const updateConcessionariaUsecase = diContainer.get(
      "UpdateConcessionariaUsecase"
    );
    expect(updateConcessionariaUsecase).toBeInstanceOf(
      UpdateConcessionariaUsecase
    );

    const destroyConcessionariaUsecase = diContainer.get(
      "DestroyConcessionariaUsecase"
    );
    expect(destroyConcessionariaUsecase).toBeInstanceOf(
      DestroyConcessionariaUsecase
    );

    const showConcessionariaUsecase = diContainer.get(
      "ShowConcessionariaUsecase"
    );
    expect(showConcessionariaUsecase).toBeInstanceOf(ShowConcessionariaUsecase);

    const listConcessionariaUsecase = diContainer.get(
      "ListConcessionariaUsecase"
    );
